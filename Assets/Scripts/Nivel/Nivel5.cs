...
public class Nivel5 : MonoBehaviour
{
    ...
    public GameObject alienTipo4Prefab;
    public Transform paredSuperior;

    public float intervaloGeneracionAlienTipo1 = 2f;
    public float intervaloGeneracionAlienTipo2 = 4f;
    public float intervaloGeneracionAlienTipo3 = 6f;
    ...
    private float tiempoProximoAlienTipo3;
    ...
    void Start()
    {
        GenerarJefeFinal(alienTipo4Prefab);
        ...
        tiempoProximoAlienTipo3 = Time.time + intervaloGeneracionAlienTipo3;
    ...
    void Update()
    {
        if (!nivelTerminado)
        {
            ...
            if (Time.time >= tiempoProximoAlienTipo3)
            {
                GenerarAlien(alienTipo3Prefab);
                tiempoProximoAlienTipo3 = Time.time + intervaloGeneracionAlienTipo3;
            }
        }
    }
    ...
    void GenerarAlien(GameObject alienPrefab)
    {
        ...
        if (alienPrefab.GetComponent<LogicaAlienTipo3>() != null)
        {
            LogicaAlienTipo3 logicaAlien = nuevoAlien.GetComponent<LogicaAlienTipo3>();
            if (logicaAlien != null)
            {
                logicaAlien.barrera = barrera;
                logicaAlien.OnBarreraColision += AlienTocoBarrera;
    ...
    void GenerarJefeFinal(GameObject jefePrefab)
    {
        float centroPantalla = Camera.main.transform.position.x;

        float posicionAjustadaY = paredSuperior.position.y + 1f;

        Vector2 posicionJefe = new Vector2(centroPantalla, posicionAjustadaY);

        GameObject jefeFinal = Instantiate(jefePrefab, posicionJefe, Quaternion.identity);

        if (jefeFinal.GetComponent<LogicaAlienTipo4>() != null)
        {
            LogicaAlienTipo4 logicaJefe = jefeFinal.GetComponent<LogicaAlienTipo4>();
            if (logicaJefe != null)
            {
                logicaJefe.barrera = barrera;
                logicaJefe.OnBarreraColision += AlienTocoBarrera;
            }
        }
    }
